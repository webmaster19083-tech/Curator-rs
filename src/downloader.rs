...
fn short_error_summary(log: &str) -> String {
    ...
    log.lines()
        ...
        .filter(|l| {
            let l = l.trim();
            !l.is_empty() && !l.contains("RequestsDependencyWarning") && !l.starts_with("  ")
        })
        .take(3)
        .collect::<Vec<_>>()
        .into_iter()
...
pub fn scan_and_index(
    ...
    let real_existing: HashSet<String> = {
        let mut stmt =
            conn.prepare("SELECT filepath FROM media WHERE source_id=? AND downloaded=1")?;
        let real_existing: HashSet<String> = stmt
    ...
    let mut placeholders_by_url: HashMap<String, i64> = {
        let mut stmt =
            conn.prepare("SELECT id, origin_url FROM media WHERE source_id=? AND downloaded=0")?;
        let placeholders: HashMap<String, i64> = stmt
    ...
    if !retired.is_empty() {
        let mut stmt =
            conn.prepare("UPDATE media SET filepath=?, filename=?, downloaded=1 WHERE id=?")?;
...
pub async fn populate_placeholders(state: Arc<AppState>, source_id: i64) {
    ...
    info!(
        "Placeholder pre-scan for source {source_id}: {} candidate item(s)",
        items.len()
    );
}
...
pub async fn run_gallery_dl_j(url: &str, timeout_s: u64) -> Result<String> {
    ...
    let result = tokio::time::timeout(std::time::Duration::from_secs(timeout_s), read_task).await;
...
async fn run_download_inner(state: Arc<AppState>, source_id: i64) {
    ...
    let archive_path =
        dunce::simplified(&archives_dir.join(format!("{slug}.sqlite3"))).to_path_buf();
    ...
    let indexer = tokio::spawn(async move {
        loop {
            ...
            let _ =
                tokio::task::spawn_blocking(move || scan_and_index(&pool, sid, &dest, &lib)).await;
        }
    });
