...
pub fn import_folder(state: &crate::AppState, folder: &Path, group_id: Option<i64>) -> Result<i64> {
    ...
    let existing: Option<i64> = conn
        .query_row("SELECT id FROM sources WHERE url=?1", [&url], |r| r.get(0))
        .optional()?;
...
pub fn sync_folder(state: &crate::AppState, id: i64, folder: &Path) -> Result<()> {
    let slug: String =
        state
            .pool
            .get()?
            .query_row("SELECT slug FROM sources WHERE id=?1", [id], |r| r.get(0))?;
    ...
    let result = (|| -> Result<()> {
        for entry in walkdir::WalkDir::new(folder).follow_links(false) {
            ...
            let ext = entry
                ...
                .unwrap_or("")
                .to_lowercase();
            if !crate::downloader::image_exts().contains(&ext.as_str())
