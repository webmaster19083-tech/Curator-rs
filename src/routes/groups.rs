...
pub async fn create(
    ...
    if let Some(pid) = body.parent_id {
        let ok = conn
            .query_row("SELECT id FROM groups WHERE id=?", [pid], |_| Ok(()))
            .is_ok();
        if !ok {
            return Err(bad_request("Parent group not found"));
        }
    }
    let id: i64 = conn
        .query_row(
    ...
    let row = conn
        .query_row("SELECT * FROM groups WHERE id=?", [id], |r| {
            Ok(json!({
                "id": r.get::<_,i64>(0)?,
    ...
    if body.clear_parent {
        ...
        if is_self_or_descendant(conn, pid, id) {
            return Err(bad_request(
                "Can't move a group into itself or one of its own subgroups",
            ));
        }
        let ok = conn
            .query_row("SELECT id FROM groups WHERE id=?", [pid], |_| Ok(()))
...
pub async fn update(
    ...
    let conn = state.pool.get().map_err(anyhow::Error::from)?;
    let exists = conn
        .query_row("SELECT id FROM groups WHERE id=?", [id], |_| Ok(()))
        .is_ok();
    if !exists {
...
pub async fn delete(
    ...
    let parent_id: Option<i64> = conn
        .query_row("SELECT parent_id FROM groups WHERE id=?", [id], |r| {
            r.get(0)
        })
...
fn is_self_or_descendant(conn: &rusqlite::Connection, candidate: i64, of: i64) -> bool {
    ...
    children
        .iter()
        .any(|&c| is_self_or_descendant(conn, candidate, c))
}
