    ...
    let row = {
        ...
        if let Some(pid) = body.parent_id {
            ...
            if !exists {
                return Err((
                    StatusCode::BAD_REQUEST,
                    Json(json!({"error": "Parent group not found"})),
                ));
            }
        }
        ...
        conn.query_row("SELECT * FROM groups WHERE id=?1", [new_id], row_to_json)
            .map_err(db_err)?
    };
        ...
        if body.clear_parent {
            ...
            if group_is_self_or_descendant(&conn, pid, id) {
                return Err((
                    StatusCode::BAD_REQUEST,
                    Json(
                        json!({"error": "Can't move a group into itself or one of its own subgroups"}),
                    ),
                ));
            }
            let exists: bool = conn
...
pub async fn delete(
    ...
    let row = conn.query_row("SELECT parent_id FROM groups WHERE id=?1", [id], |r| {
