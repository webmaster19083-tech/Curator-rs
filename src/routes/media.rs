        ...
        .filter_map(
            |(
                ...
                let effective: HashSet<&String> =
                    own_tags.iter().chain(group_tags.iter()).collect();

                if let Some(ref tf) = tag_filter {
                    if !effective.contains(tf) {
                        return None;
                    }
                }
...
pub async fn set_rating(
    ...
    let exists: bool = conn
        .query_row("SELECT id FROM media WHERE id=?", [id], |_| Ok(()))
        .is_ok();
    if !exists {
...
pub fn get_or_create_tag(conn: &rusqlite::Connection, name: &str) -> Result<i64, AppError> {
    ...
    if let Ok(id) = conn.query_row("SELECT id FROM tags WHERE name=?", [&name], |r| {
        r.get::<_, i64>(0)
    }) {
        return Ok(id);
    }
